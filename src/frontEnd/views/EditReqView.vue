...
const handleSubmit = async () => {
  if (router.currentRoute.value.path.includes("edit")) {
    await jobStore.updateJobStore(form).then(async () => {
    ...
    <Card class="mx-2 md:mx-4 lg:mx-auto mb-20 lg:mb-10 max-w-4xl">
      <CardHeader class="text-center">
        <CardDescription class="text-center text-lg font-normal">
          <h1 v-if="router.currentRoute.value.path.includes('edit')">
      ...
      <CardContent>
        <div class="grid gap-4">
          <div class="grid gap-2">
            <Label for="categories" class="text-sky-950"
              >Categoria
          ...
          <div class="grid gap-2">
            <Label for="title" class="text-sky-950"
              >Titolo
          ...
          <div class="grid gap-2">
            <Label for="description" class="text-sky-950"
              >Descrizione
          ...
          <p class="text-xs italic text-center text-sky-950 opacity-70">
            Cerca di essere specifico nella tua richiesta così che il Worker
            possa capire di cosa hai bisogno.
          </p>
        </div>
        <div class="flex justify-center gap-4">
          <Button
            @click="router.replace('/jobs')"
            class="mt-4 primary-btn hover:!text-red-500"
            type="reset"
            >Annulla</Button
          >
          <Button class="mt-4 primary-btn hover:!text-green-500" type="submit"
            >Invia</Button
          >
        </div>
      </CardContent>
